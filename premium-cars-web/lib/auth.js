...
/* Rahasia penanda tangan sesi.
   ...
   Sebelumnya nilai ini jatuh ke string tetap saat AUTH_SECRET tidak di-set.
   Karena string itu ada di dalam source (repo publik), siapa pun bisa
   menempa cookie sesi berisi email admin dan mendapat akses penuh ke
   /api/admin. Di produksi kini TIDAK ada cadangan: tanpa AUTH_SECRET,
   sesi tidak bisa dibuat maupun diverifikasi (admin tertutup rapat),
   sementara halaman publik tetap berjalan normal. */
const DEV_SECRET = "dev-secret-premium-cars-ganti-di-produksi";
const IS_PROD = process.env.NODE_ENV === "production";
const SECRET = process.env.AUTH_SECRET || (IS_PROD ? "" : DEV_SECRET);

if (IS_PROD && !SECRET) {
  console.error(
    "[auth] AUTH_SECRET belum di-set — login & sesi dinonaktifkan. " +
      "Set AUTH_SECRET di environment untuk mengaktifkan kembali."
  );
}
